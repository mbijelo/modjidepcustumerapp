...
class _NotifyScreenState extends State<NotifyScreen> {
  ...
  _body(){
    ...
    for (var item in messages){
      ...
      list.add(Card48(
        //text: item.body,
        text: item.body == "État actuel : Accept" || item.body == "Now status: Accept" || item.body == "الآن الحالة: Accept" ? strings.get(158) + " " + strings.get(59)
            : item.body == "État actuel : Ready" || item.body == "Now status: Ready" || item.body == "الآن الحالة: Ready" ? strings.get(158)  + " " + strings.get(295)
            : item.body == "État actuel : Cancel" || item.body == "Now status: Cancel" || item.body == "الآن الحالة: Cancel" ? strings.get(158)  + " " + strings.get(62)
            : item.body,
        text2: appSettings.getDateTimeString(item.time),
        text3: item.title == "Booking status was changed" || item.title == "Le statut de la réservation a été modifié" || item.title == "تم تغيير حالة الحجز" ? strings.get(159) :  item.title,
        callback: () async {
    ...
    if (_count == 0) {
      ...
      list.add(Center(child: Text(strings.get(300), style: theme.style18W800GreyNotif,),)); /// "Not found ...",
    }
