...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  _titleSmall(){
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          FittedBox(fit: BoxFit.scaleDown, child: Text(_text1, style: theme.style16W800White,)),
          SizedBox(height: 3,),
          FittedBox(fit: BoxFit.scaleDown, child: Text(_text2, style: theme.style10W600White)),
      ],
    )
    );
  }
  ...
  _body(){
    ...
    while(i < _data.length){
      if(_data[i] != "²") {
        CNameFrench = CNameFrench+_data[i];
      }
      else{
        break;
      }
    ...
    while(i < _data.length){
      if(_data[i] != '²') {
        CNameEnglish = CNameEnglish+_data[i];
      }
      else{
        break;
      }
    ...
    CNameArabic = "";
    while(i < _data.length){
      CNameArabic = CNameArabic+_data[i];
      i++;
    }
    dprint("CNameArabic : " + CNameArabic);


    if (strings.locale  == "fr")
      _data = CNameFrench;
