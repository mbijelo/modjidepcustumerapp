...
import 'package:ondemandservice/ui/reviews1.dart';
  ...
  _body2(){
    return Stack(
        children: [
          ...
          appbar1((_scroller > 1) ? Colors.black54 :
          (theme.darkMode) ? Colors.white : Colors.white,
              (theme.darkMode) ? Colors.white : Colors.white,
...
/*
    list.add(
      Container(
        margin: EdgeInsets.only(bottom: 0, left: 0, right: 50, top: 6),
        alignment: strings.direction == TextDirection.ltr
            ? Alignment.topRight
            : Alignment.topLeft,
        child: IconButton(
...
*/
    list.add(Container(
        ...
        child: Card50forProvider(
          ...
          textt: strings.get(294),
          callllBack: (){
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => Reviews1Screen(mainModel: _mainModel),
              ),
            );
          },
        )
...
/*
    ...
    */
    ...
    for (var item in product) {
      ...
      i = 0;
      CNameFrench = "";
      while(i < getCategoryNameById(item.category[0]).length){
        if(getCategoryNameById(item.category[0])[i] != "²") {
          CNameFrench = CNameFrench+getCategoryNameById(item.category[0])[i];
        }
        else{
          break;
        }
      ...
      while(i < getCategoryNameById(item.category[0]).length){
        if(getCategoryNameById(item.category[0])[i] != '²') {
          CNameEnglish = CNameEnglish+getCategoryNameById(item.category[0])[i];
        }
        else{
          break;
        }
      ...
      CNameArabic = "";
      while(i < getCategoryNameById(item.category[0]).length){
        CNameArabic = CNameArabic+getCategoryNameById(item.category[0])[i];
        i++;
      }
      ...
      list.add(Container(
        ...
        child: Text(
          strings.locale  == "en" ? CNameEnglish : strings.locale == "fr" ? CNameFrench : strings.locale == "ar" ? CNameArabic : "",/// "Services",
          style: theme.style14W800,),
      ));
